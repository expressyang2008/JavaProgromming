 ...
 * @description: This program translates a line of text using a letter-substitution  cipher.
 * @version: 0.1.0
 * @author: Shawn Yang
...
public class LetterSubstitutionCipher extends ConsoleProgram{
    public void run() {
        println("This program implements a letter-substitution cipher."); String key = readKey();
        String plaintext = readLine("Plaintext: ");
        String ciphertext = encrypt(plaintext, key);
        String decryption = decrypt(ciphertext, key); println("Ciphertext: " + ciphertext);
    ...
    private String readKey() {
        String key = readLine("Enter 26-letter key: "); while (!isLegalKey(key)) {
            println("That key is not legal.");
            key = readLine("Reenter key: ");
        }
        return key; }
    /**
     * Checks to see whether a key is legal, which means it meets * the following conditions:
     * (1) It is 26 characters long,
     * (2) It contains only uppercase letters, and
     * (3) It has no duplicated letters.
     */
    private boolean isLegalKey(String key) { if (key.length() != 26) return false; for (int i = 0; i < 26; i++) {
        char ch = key.charAt(i);
        if (!Character.isUpperCase(ch)) return false; for (int j = i + 1; j < 26; j++) {
    ...
    private String encrypt(String str, String key) { String result = "";
    ...
    private char encryptCharacter(char ch, String key) { if (Character.isLetter(ch)) {
        ch = key.charAt(Character.toUpperCase(ch) - 'A'); }
        return ch; }
    /**
     * Decrypts a string according to the key. */
    private String decrypt(String str, String key) { String result = "";
        for (int i = 0; i < str.length(); i++) {
    ...
    private char decryptCharacter(char ch, String key) { int index = key.indexOf(ch);
        if (index != -1) {
            ch = (char) ('A' + index);
        }
        return ch; }
